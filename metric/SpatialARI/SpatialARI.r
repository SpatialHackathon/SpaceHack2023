...
if (!is.na(opt$coordinates)) {
  coordinates_file <- opt$coordinates
}

ground_truth <- read.delim(groundtruth_file, sep="\t", row.names=1)
labels <- read.delim(label_file, sep="\t", row.names=1)
coordinates <- read.delim(coordinates_file, sep="\t", row.names=1)

common_index <- intersect(rownames(ground_truth), rownames(labels))
...
#ground_truth$label
#ground_truth <- ground_truth[rownames(labels),]
#labels$label
#ground_truth

!any(is.na(ground_truth$label))

metric <- getSpatialExternalMetrics(true=factor(ground_truth[,1]),
                          pred=factor(labels[,1]),
                          location=coordinates[,c("x", "y")],
                          level="dataset",fuzzy_true=TRUE, fuzzy_pred=FALSE,metrics=c("SpatialARI"))
metric <- metric[1,1]
metric
